 ...
 *     re-deriving expectations from its two inputs — the PREVIOUS-release baseline
 *     JSON and the current dataset markdown.
 *
 * Verifies the transformation, not parity with the hand-authored target release.
 * Exits non-zero on any failure. Honors PREVIOUS_VERSION / NEW_VERSION / DATASET_FOLDER.
 */
...
function structuralChecks(check, warn) {
  ...
  let dupIds = 0, dupNums = 0, unsorted = 0, dangling = 0, inconsistent = 0, entities = 0, prefixMismatch = 0;
  for (const file of files) {
    const rules = JSON.parse(fs.readFileSync(file, 'utf8'));
    ...
    for (const id of ids) {
      ...
      if (r.Function === 'Composite') {
        const items = (r.ValidationCriteria.Requirement.Items || []).map((i) => i.ModelRuleId).join(',');
        if (items !== r.ValidationCriteria.Dependencies.join(',')) { inconsistent++; console.log('     Items!=Deps', id); }
      }
  ...
  check(inconsistent === 0, 'Composite Items==Dependencies and Keyword==MustSatisfy keyword');
...
function datasetAudit(check) {
  ...
  const baselineDir = path.join(RELEASES_DIR, PREVIOUS_VERSION, 'model_rules', 'datasets', DATASET_FOLDER);
  const baseline = {};
  ...
  const expectedIds = new Set([...Object.keys(active), ...Object.keys(baseline).filter((id) => !expected.has(normalize(baseline[id].ValidationCriteria.MustSatisfy)))]);
